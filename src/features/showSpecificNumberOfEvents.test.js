...
const feature = loadFeature('./src/features/showSpecificNumberOfEvents.feature');
...
defineFeature(feature, test => {
  ...
  test("When user hasn't specified a number the default number is 32", ({ given, when, then }) => {
    ...
    then(/^the default number of displayed events will be (\d+)$/, async () => {
      const AppDOM = AppComponent.container.firstChild;
      const EventListDOM = AppDOM.querySelector('#event-list');
      await waitFor(() => {
        const EventListItems = within(EventListDOM).queryAllByRole('listitem');
        expect(EventListItems).toEqual(32);
      })
    });
  });
